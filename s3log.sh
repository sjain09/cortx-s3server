...
filename=
if [ $info -eq 1 ]; then
	filename=/var/log/seagate/s3/s3server.INFO
fi
...
if [ $warn -eq 1 ]; then
	filename=/var/log/seagate/s3/s3server.WARNING
fi

if [ $error -eq 1 ]; then
	filename=/var/log/seagate/s3/s3server.ERROR
fi

if [ $auth -eq 1 ]; then
	filename=/var/log/seagate/auth/server/app.log
fi

if [ $bd -eq 1 ]; then
	filename=/var/log/seagate/s3/s3backgrounddelete/object_recovery_processor.log
	if [  $bd_schedular -eq 1 ]; then
		filename=/var/log/seagate/s3/s3backgrounddelete/object_recovery_scheduler.log
	fi
fi
