...
result = AwsTest('Aws can get object without specifing versionId').get_object("seagatebucket", "1kfile", version_id=versionId)\
    .execute_test().command_is_successful()

result = AwsTest('Aws can not get object with wrong versionId').get_object("seagatebucket", "1kfile", version_id=versionId)\
    .execute_test().command_should_fail().command_error_should_have("InvalidArgument").command_error_should_have("Invalid version id specified")

result = AwsTest('Aws can not get object with empty versionId').get_object("seagatebucket", "1kfile", version_id="empty")\
    .execute_test().command_should_fail().command_error_should_have("InvalidArgument").command_error_should_have("Version id cannot be the empty string")

#******** GetObject with Bucket versioning suspended ********
AwsTest('Aws can suspend versioning on bucket').put_bucket_versioning("seagatebucket", "Suspended").execute_test().command_is_successful()
