...
S3GetBucketVersioningAction::S3GetBucketVersioningAction(
    ...
    std::shared_ptr<S3BucketMetadataFactory> bucket_meta_fatory)
    : S3BucketAction(std::move(req), std::move(bucket_meta_fatory), false) {
...
void S3GetBucketVersioningAction::setup_steps() {
    s3_log(S3_LOG_DEBUG, request_id, "Seting up the action.\n");
    ACTION_TASK_ADD(S3GetBucketVersioningAction::send_response_to_s3_client, this);
}

void S3GetBucketVersioningAction::fetch_bucket_info_failed() {
    s3_log(S3_LOG_DEBUG, stripped_request_id, "%s Entry\n", __func__);
    if(bucket_metadata->get_state() == S3BucketMetadataState::missing) {
        s3_log(S3_LOG_WARN, request_id, "Bukcet Not Found\n");
        set_s3_error("NoSuchBucket");
    } else if (bucket_metadata->get_state() == 
                S3BucketMetadataState::failed_to_launch) {
        s3_log(S3_LOG_ERROR, request_id, 
                "Bukcet metadata opretation failed due to pre-launch failure\n");
...
void S3GetBucketVersioningAction::send_response_to_s3_client(){
    s3_log(S3_LOG_DEBUG, stripped_request_id, "%s Entry\n", __func__);
    if (reject_if_shutting_down() || is_error_state() 
            || !get_s3_error_code().empty()) {
        ...
        std::string response_xml = bucket_metadata->get_versioning_status_as_xml();
        request->set_out_header_value("Content-Type","application/xml");
        request->set_out_header_value("Content-Length", 
            std::to_string(response_xml.length()));
